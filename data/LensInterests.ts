...
async function getLensInterests(query: w3dataRequest) {
...
const getInterestsTitle = (interests: string[]) => {
  const firstInterest = interests[0].substring(interests[0].lastIndexOf('_') + 1, interests[0].length)
  if (interests.length === 1) {
    return `*${firstInterest}*`.toLowerCase()
  }
  return `*${firstInterest}* & *${interests[1].substring(interests[1].lastIndexOf('_') + 1, interests[1].length)}*`.toLowerCase()
}
